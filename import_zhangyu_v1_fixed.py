"""Import Zhang Yu 8-exam-set Volume 1 questions with proper IDs."""
import json
import os
from pathlib import Path

STORAGE_DIR = 'storage'
DATA_DIR = Path(STORAGE_DIR) / "questions" / "simulations"  # 模拟卷存储位置
...
def import_all():
    ...
    if volume not in index["categories"][category]:
        index["categories"][category][volume] = {}
    
    for qtype, questions in QUESTIONS.items():
        # 初始化题型分类
        if qtype not in index["categories"][category][volume]:
            index["categories"][category][volume][qtype] = []
        
        for q in questions:
            # 使用题目编号作为ID的一部分
            qid = f"2026-26宇哥八套卷-{q['no']:03d}"
            ...
            # 写入数据文件
            save_json(data_path, item)
            
            # 更新分类索引
            index["categories"][category][volume][qtype].append(qid)
            
            # 更新知识点索引
            for kp in q["knowledge_points"]:
                if kp not in index["knowledge_index"]:
                    index["knowledge_index"][kp] = []
                if qid not in index["knowledge_index"][kp]:
                    index["knowledge_index"][kp].append(qid)
            
            # 更新难度索引
            diff = q["difficulty"]
            if diff not in index["difficulty_index"]:
                index["difficulty_index"][diff] = []
            if qid not in index["difficulty_index"][diff]:
                index["difficulty_index"][diff].append(qid)
            
            index["metadata"]["total_questions"] += 1
            total_imported += 1
            print(f"  OK: {qid} ({qtype} #{q['no']})")
        ...
        if isinstance(cat_data, dict):
            for key in cat_data.keys():
                try:
                    ...
                    pass
    index["metadata"]["years_covered"] = sorted(list(years))
    
    # 保存索引
    save_json(INDEX_PATH, index)
    
    print(f"\nDone. Imported: {total_imported}")
    print(f"DB now has {index['metadata']['total_questions']} questions")
